...
const main = async () => {
  try {
    ...
    // 获取该职位详情
    const PositioningStrategyJobDetail = By.xpath(
      '//*[@id="wrap"]/div[2]/div[3]/div/div[2]/div[1]/div[3]'
    );
    await waitElementAppera(
      PositioningStrategyJobDetail,
      webDriver,
      until,
      "职位详情"
    );
    const jobDetailElement = await findElement(
      PositioningStrategyJobDetail,
      webDriver,
      "职位详情"
    );
    ...
    const helloContent = await getHelloContent(
      openai,
      jobDetailContent,
      personalDes
    );

    console.log(helloContent, "ai生成招呼语");
    ...
    // 找到聊天弹窗中的输入框
    const PositioningStrategyChatModalInput = By.xpath(
      "//*[contains(@class, 'dialog-wrap')]/div[2]/div[2]/div/div[1]/div[2]/textarea"
    );
    await waitElementAppera(
      PositioningStrategyChatModalInput,
      webDriver,
      until,
      "聊天弹窗输入框"
    );
    const chatModalInputElement = await findElement(
      PositioningStrategyChatModalInput,
      webDriver,
      "聊天弹窗输入框"
    );
    ...
    const PositioningStrategySendButton = By.xpath(
      "//*[contains(@class, 'dialog-wrap')]/div[2]/div[2]/div/div[1]/div[2]/div"
    );
    await waitElementAppera(
      PositioningStrategySendButton,
      webDriver,
      until,
      "发送按钮"
    );
    const sendButtonElement = await findElement(
      PositioningStrategySendButton,
      webDriver,
      "发送按钮"
    );
